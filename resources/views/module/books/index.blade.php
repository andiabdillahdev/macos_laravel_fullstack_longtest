...
@section('button')
    <button class="btn btn-primary btn-sm " data-kt-drawer-show="true" data-kt-drawer-target="#side_form" id="button-side-form"><i class="fa fa-plus-circle" style="color:#ffffff" aria-hidden="true"></i> Tambah Data Buku</button>
@endsection
@section('content')
        ...
        <div class="card-body hover-scroll-overlay-y">
           <form class="form-data">
                ...
                <div class="mb-10">
                    ...
                    <input type="text" id="judul" class="form-control" name="title" placeholder="Masukkan judul">
                    <small class="text-danger title_error"></small>
                </div>

                <div class="mb-10">
                    <label class="form-label">Pengarang</label>
                    <input type="text" id="author" class="form-control" name="author" placeholder="Masukkan author">
                    <small class="text-danger author_error"></small>
                </div>

                <div class="mb-10">
                    <label class="form-label">Tahun</label>
                    <input type="text" id="year" class="form-control" name="year" placeholder="Masukkan year">
                    <small class="text-danger year_error"></small>
                </div>
...
<script>
    ...
    $(document).on('submit', ".form-data", function(e){
        ...
        if (type == 'add') {
            control.submitForm('/books/store','Tambah','Buku','POST');
        }else{
            let id = $("input[name='id']").val();
            control.submitForm('/books/update/'+id,'Update','Buku', 'PUT');
        }
    });

    $(document).on('click','.button-update', function (e) {
        e.preventDefault();
        let url = '/books/show/'+$(this).attr('data-id');
        control.overlay_form('Update','Data Buku', url);
    })

       $(document).on('click','.btn-delete', function (e) {
        e.preventDefault();
        let url = '/books/delete/'+$(this).attr('data-id');
    ...
    $(function () {
        ...
        control.initDatatable('/books/datatable-list',columns,columnDefs);
    })
</script>
@endsection